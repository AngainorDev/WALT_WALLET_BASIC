...
STATIC mp_obj_t mod_trezorcrypto_AES_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    if (n_args > 2) {
        ...
        if (iv.len != AES_BLOCK_SIZE) {
            mp_raise_ValueError(MP_ERROR_TEXT("Invalid length of initialization vector (has to be 128 bits)"));
        }
    ...
    switch (key.len) {
        default:
            mp_raise_ValueError(MP_ERROR_TEXT("Invalid/unsupported key length"));
            break;
#ifdef AES_128
        case 16:
