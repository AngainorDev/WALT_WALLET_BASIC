...
STATIC void check_len32(const uint8_t len) {
    if (len != 32) {
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_verify(mp_obj_t public_key, mp_obj_t signature, mp_obj_t message) {
    ...
    if (sig.len != 64) {
        mp_raise_ValueError(MP_ERROR_TEXT("Invalid length of signature"));
    }
    if (msg.len == 0) {
        mp_raise_ValueError(MP_ERROR_TEXT("Empty data to verify"));
    }
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_combine_publickeys(mp_obj_t public_keys) {
    ...
    if (pklen > 15) {
        mp_raise_ValueError(MP_ERROR_TEXT("Can't combine more than 15 public keys"));
    }
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_combine_signatures(mp_obj_t R, mp_obj_t signatures) {
    ...
    if (siglen > 15) {
        mp_raise_ValueError(MP_ERROR_TEXT("Can't combine more than 15 COSI signatures"));
    }
    mp_buffer_info_t buf;
...
STATIC mp_obj_t mod_trezorcrypto_ed25519_cosi_sign(size_t n_args, const mp_obj_t *args) {
    ...
    if (pk.len != 32) {
        mp_raise_ValueError(MP_ERROR_TEXT("Invalid length of aggregated public key"));
    }
    uint8_t out[32];
