...
STATIC mp_obj_t mod_tcc_bip39_lookup_nth(mp_obj_t idx_obj)
{
    ...
    if(idx < 0 || idx > 0x7ff) {
        mp_raise_ValueError(MP_ERROR_TEXT("wordlist range"));
    }
...
STATIC mp_obj_t mod_tcc_bip39_lookup_word(mp_obj_t word)
{
    ...
    mp_raise_ValueError(MP_ERROR_TEXT("Unknown word"));
}
...
STATIC mp_obj_t mod_trezorcrypto_bip39_find_word(mp_obj_t prefix)
{
    ...
    if (pfx.len == 0) {
        mp_raise_ValueError(MP_ERROR_TEXT("Invalid word prefix"));
    }
...
STATIC mp_obj_t mod_trezorcrypto_bip39_generate(mp_obj_t strength) {
    ...
    if (bits % 32 || bits < 128 || bits > 256) {
        mp_raise_ValueError(MP_ERROR_TEXT("Invalid bit strength (only 128, 160, 192, 224 and 256 values are allowed)"));
    }
...
STATIC mp_obj_t mod_trezorcrypto_bip39_from_data(mp_obj_t data) {
    ...
    if (bin.len % 4 || bin.len < 16 || bin.len > 32) {
        mp_raise_ValueError(MP_ERROR_TEXT("Invalid data length (only 16, 20, 24, 28 and 32 bytes are allowed)"));
    }
    char mnemo[240];
